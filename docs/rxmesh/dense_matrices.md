...
Dense matrices are often constructed from mesh attributes, e.g., vertex coordinates or physical quantities. Attributes can be directly converted to dense matrices and vice versa (see [Working with Attributes](working_with_attributes.md)).

For small matrix operations—e.g., working with 3×3 matrices for SVD—we recommend using the attribute interface's `to_eigen` and `to_glm` methods (also documented in [Working with Attributes](working_with_attributes.md)). These are optimized for use within GPU kernels and avoid unnecessary memory transfers.
